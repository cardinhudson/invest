...
def processar_renda_fixa(xls, usuario, mes_ano):
    ...
    for nome in xls.sheet_names:
        # Busca simples como em proventos - verificar se contém "Renda" OU "Fixa"
        if "Renda" in nome or "Fixa" in nome or "renda" in nome or "fixa" in nome:
...
def salvar_tipo_parquet(df_tipo, path):
    ...
    if os.path.exists(path):
        ...
        # Remover dados anteriores do mesmo usuário/mês (overwrite)
        if "Usuário" in df_tipo.columns and "Mês/Ano" in df_tipo.columns:
            ...
            print(f"Removidos dados anteriores de {usuario} - {mes_ano}")
    ...
    print(f"✅ Arquivo salvo com sucesso: {path} ({len(df_final)} linhas totais)")
